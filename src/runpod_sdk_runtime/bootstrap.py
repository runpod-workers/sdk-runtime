...
sdk and every python dependency, resolved at deploy time for the worker
platform. this entrypoint only has to make that environment visible and
exec the worker.
...
deployed mode (FLASH_RESOURCE_NAME set):
  ...
  serve    exec the worker runtime with the vendored sdk environment

live mode (no resource name, `rp dev`):
  no artifact exists; source arrives per request. the runtime images
  bake the runpod sdk in, custom images get it installed here.
...
stdlib only: this file runs on the image's python before any artifact
environment exists. runtime images bake it in, and custom images install
the runtime package before starting it via dockerArgs.
"""
...
WORKER_MODULES = {
    "queue": "runpod_sdk_runtime.queue.worker",
    "api": "runpod_sdk_runtime.api.server",
}

SDK_MODULES = {
    "queue": ("runpod.apps", "runpod.serverless"),
    "api": ("runpod.apps", "fastapi", "uvicorn"),
}
...
def _locate():
    ...
    _log(f"artifact extracted to {APP_DIR} in {time.monotonic() - started:.1f}s")
    return APP_DIR
...
def _attach(app_dir):
    ...
    if os.path.isdir(env_dir):
        ...
        _log(
            f"artifact has no vendored env at {env_dir}; falling back to image packages"
        )
    return paths
...
def _sdk_importable(paths):
    """can the worker import its sdk dependencies with these paths leading?"""
    modules = SDK_MODULES[_runtime_kind()]
    probe = (
        "import sys; sys.path[:0] = sys.argv[1:]; import importlib; "
...
def _ensure_sdk_installed():
    """ensure live workers can import the runpod sdk and server deps."""
    ok, _ = _sdk_importable([])
    if ok:
        return
    spec = os.environ.get("RUNPOD_PACKAGE_SPEC", "runpod")
    _log(f"worker sdk not in image, installing {spec}")
    packages = [spec, "cloudpickle"]
    if _runtime_kind() == "api":
        packages.extend(("fastapi[standard]", "uvicorn>=0.30"))
    _pip_install(packages, "runtime")
    ok, err = _sdk_importable([])
    if not ok:
        raise PhaseError(
            "runtime",
            f"installed {spec} but the worker sdk dependencies are still "
...
def _queue_error_loop(error):
    ...
    while True:
        try:
            req = urllib.request.Request(get_url, headers={"Authorization": api_key})
...
def main():
    try:
        if _resource_name():
            ...
            ok, err = _sdk_importable(paths)
            if not ok:
                raise PhaseError(
                    "attach",
                    f"worker sdk dependencies are not importable from the "
                    f"artifact env or the image: {err}",
                )
            _serve(paths)
        else:
