...
def _mount_routes(app: Any, handle, instance) -> None:
    ...
    for route in handle.spec.routes:
        ...
        handler_name = getattr(route, "handler_name", None) or route["handler"]
...
def _install_live_dependencies(request: dict) -> None:
    ...
    """
    from runpod_sdk_runtime.executor import _install, _install_system
...
async def _materialize_live_api(source: str, resource: str) -> Any:
    ...
    from runpod_sdk_runtime.executor import _materialize_source
    ...
    for value in namespace.values():
        if isinstance(value, ApiHandle) and value.spec.name == resource:
            ...
            break
    if handle is None:
        raise RuntimeError(f"no @app.api resource named '{resource}' in shipped module")
