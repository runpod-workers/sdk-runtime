...
def _load_deployed_handle():
    ...
    for attr in vars(module).values():
        if isinstance(attr, FunctionHandle) and attr.spec.name == name:
            return attr
    raise RuntimeError(
...
async def _live_handler(job: dict):
    ...
    from runpod_sdk_runtime.executor import (
        execute_request,
        resolve_request,
        serialize_chunk,
    )
...
def main() -> None:
    if _is_deployed():
        ...
        _run_init(handle)
        runpod.serverless.start(_worker_config(_make_deployed_handler(handle)))
    else:
