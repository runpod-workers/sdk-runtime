...
from runpod_sdk_runtime.api import server
...
class TestFactoryApi:
    def test_asgi_factory_served(self, tmp_path, monkeypatch):
        ...
        manifest = {
            "version": 1,
            "app": "api-test",
            "resources": [{"kind": "api", "name": "web", "module": "main_factory"}],
        }
        (tmp_path / "runpod_manifest.json").write_text(json.dumps(manifest))
        monkeypatch.setattr(server, "APP_DIR", str(tmp_path))
...
class TestLiveDispatcher:
    def test_sync_materializes_routes(self):
        ...
        from runpod_sdk_runtime.api.server import _build_live_app
        ...
        r = client.post("/_runpod/sync", json={"source": source, "resource": "Counter"})
    ...
    def test_sync_installs_dependencies_before_materializing(self):
        ...
        client = TestClient(_build_live_app())
        with (
            patch("runpod_sdk_runtime.executor._install", return_value=None) as install,
            patch(
                "runpod_sdk_runtime.executor._install_system",
                return_value=None,
            ) as install_system,
        ):
            r = client.post(
