"""tests for the queue and api bootstrap."""

import json

import pytest

from runpod_sdk_runtime import bootstrap
...
class TestSystemPhase:
    ...
    def test_missing_apt_fails_with_phase(self, monkeypatch):
        ...
        manifest = {"resources": [{"name": "q", "systemDependencies": ["ffmpeg"]}]}
        with pytest.raises(bootstrap.PhaseError) as exc_info:
    ...
    def test_other_resources_deps_ignored(self, monkeypatch):
        monkeypatch.setenv("FLASH_RESOURCE_NAME", "q")
        manifest = {"resources": [{"name": "other", "systemDependencies": ["ffmpeg"]}]}
...
class TestBootstrapIsStdlibOnly:
    def test_no_third_party_imports(self):
        ...
        # module-level imports only: function-local imports in the sdk
        # availability probe run after installation
        imports = set()
...
class TestLiveSdkInstall:
    def test_importable_sdk_no_op(self, monkeypatch):
        monkeypatch.setattr(bootstrap, "_sdk_importable", lambda paths: (True, ""))
        installs = []
        monkeypatch.setattr(
            bootstrap, "_pip_install", lambda pkgs, phase: installs.append(pkgs)
        )
        bootstrap._ensure_sdk_installed()
        assert installs == []

    def test_installs_package_spec(self, monkeypatch):
        probes = iter([(False, "no module"), (True, "")])
        monkeypatch.setattr(bootstrap, "_sdk_importable", lambda paths: next(probes))
        installs = []
        monkeypatch.setattr(
            bootstrap, "_pip_install", lambda pkgs, phase: installs.append(pkgs)
        )
    ...
    def test_api_kind_adds_server_dependencies(self, monkeypatch):
        probes = iter([(False, "no module"), (True, "")])
        monkeypatch.setattr(bootstrap, "_sdk_importable", lambda paths: next(probes))
        installs = []
        monkeypatch.setattr(
        ...
        bootstrap._ensure_sdk_installed()
        assert installs == [
            ["runpod", "cloudpickle", "fastapi[standard]", "uvicorn>=0.30"]
        ]

    def test_install_failure_raises_phase(self, monkeypatch):
        monkeypatch.setattr(
            bootstrap, "_sdk_importable", lambda paths: (False, "still broken")
        )
        monkeypatch.setattr(bootstrap, "_pip_install", lambda pkgs, phase: None)
        with pytest.raises(bootstrap.PhaseError) as exc_info:
            bootstrap._ensure_sdk_installed()
...
class TestMain:
    ...
    def test_phase_error_reports(self, monkeypatch):
        ...
        monkeypatch.setattr(
            bootstrap,
            "_locate",
            lambda: (_ for _ in ()).throw(bootstrap.PhaseError("locate", "gone")),
        )
        reported = []
        monkeypatch.setattr(bootstrap, "_report_error", lambda e: reported.append(e))
        bootstrap.main()
    ...
    def test_live_path(self, monkeypatch):
        ...
        monkeypatch.setattr(bootstrap, "_ensure_sdk_installed", lambda: None)
        served = []
        monkeypatch.setattr(bootstrap, "_serve", lambda paths: served.append(paths))
        bootstrap.main()
        assert served == [[]]
